...
class Prenda {
    ...
    public function create($data) {
        $query = "INSERT INTO prendas (MarcaID, Nombre,  Precio, Stock) VALUES (:marca_id, :nombre, :precio, :stock)";
        $stmt = $this->conn->prepare($query);
    ...
    public function update($id, $data) {
        $query = "UPDATE prendas SET MarcaID = :marca_id, Nombre = :nombre, Precio = :precio, Stock = :stock WHERE PrendaID = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':marca_id', $data['marca_id'], PDO::PARAM_INT);
        $stmt->bindParam(':nombre', $data['nombre']);
        $stmt->bindParam(':precio', $data['precio']);
